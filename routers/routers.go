...
func GetCode(c *gin.Context) {
	var e PushEvent
	err := c.ShouldBindBodyWith(&e, binding.JSON)
	...
	commitID := e.After
	fmt.Println("仓库地址", repoURL)
	fmt.Println("push 后的 commitId", commitID)

	LocalRepositoryDir := "E:\\GoLandProject\\GitHubWebHookDemo"
	RepositoryLocation := LocalRepositoryDir + "\\" + e.Repository.Name
	err = os.RemoveAll(RepositoryLocation)
	if err != nil {
		panic(err)
		return
	}
	...
	fmt.Println("进入目录")
	err = os.Chdir(RepositoryLocation)
	if err != nil {
		panic(err)
		return
	}

	fmt.Println("执行 git checkout")
	// 构建git checkout命令
